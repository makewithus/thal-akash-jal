...
for filename, data in services.items():
    ...
    main_content_pattern = re.compile(r'(<div class="w-richtext">)(.*?)(</div>\s*</div>\s*<div data-w-id=)', re.DOTALL)
    new_richtext = r'\1\n' + data['mainContent'] + r'\n                            \3'
    if main_content_pattern.search(content):
        content = main_content_pattern.sub(new_richtext, content)
    else:
        print(f"Warning: Could not find w-richtext in {filename}")
        
    # Replace Benefits
    benefits_html = ""
    for b in data['benefits']:
        benefits_html += f"""                                <div class="solutions__features-item">
    ...
    benefits_pattern = re.compile(r'(<div class="solution__features-wrapper">)(.*?)(</div>\s*</div>\s*</div>\s*<div class="spacer__main"></div>)', re.DOTALL)
    new_benefits = r'\1\n' + benefits_html + r'                            \3'
    if benefits_pattern.search(content):
        content = benefits_pattern.sub(new_benefits, content)
    else:
        print(f"Warning: Could not find solution__features-wrapper in {filename}")
        
    # Replace form hidden input value
    # E.g. <input name="Expertise Name" value="...">
    form_input_pattern = re.compile(r'(<input\s+name="Expertise Name"\s+value=")(.*?)(">)')
    if form_input_pattern.search(content):
        content = form_input_pattern.sub(r'\g<1>' + data['name'] + r'\g<3>', content)
    else:
        print(f"Warning: Could not find form input Expertise Name in {filename}")
